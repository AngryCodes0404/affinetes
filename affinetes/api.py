...
def load_env(
    ...
    **backend_kwargs,
    ...
    try:
        ...
        if connect_only:
            if not container_name:
                raise ValidationError(
                    "container_name is required when connect_only=True"
                )
            if replicas != 1:
                raise ValidationError(
                    "connect_only mode only supports single instance (replicas=1)"
                )
        else:
        ...
        logger.debug(
            f"Loading '{image or container_name or 'url-service'}' in {mode} mode (replicas={replicas}, connect_only={connect_only})"
        )
...
def _load_single_instance(
    ...
    **backend_kwargs,
    ...
    if mode == "docker":
        ...
        backend = BasilicaBackend(image=image, **backend_kwargs)
    elif mode == "url":
        ...
        backend = URLBackend(**backend_kwargs)
    else:
        raise ValidationError(
...
def _load_multi_instance(
    ...
    **backend_kwargs,
    ...
    try:
        ...
        pool = InstancePool(
            instances=instances, load_balance_strategy=load_balance, pool_name=pool_name
        )
